...
function scrub(data, type){
  var _return = {},
      config = type || config.structure.types[data.type];
  
  _return.template = config.template+'.hbs';
  _return.handle = data.fields.title.toLowerCase().replace(/[\s\,\-]/g,'-');
  _return.path = '/' + config.rootPath + '/' + _return.handle + '/index.html';
  _return.permalink = storage.site.meta.url+_return.path;

  for (key in data.fields){
    _return[key] = data.fields[key] || data.fields[key]['en-US']
  }

...
function prepContentTypePages(type){
  var configType = config.structure.types[type];
  storage[type].forEach(function(typeData){
    var locals = scrub(typeData, configType);
    assemble.page(
      '.'+locals.path, 
      {
        content: getTemplate(locals.template), 
        locals: locals
      }
    );
  });

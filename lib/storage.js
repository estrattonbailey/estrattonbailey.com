...
function normalizeContentful(partial){
  ...
  if (typeSettings && typeSettings.hasPage){
    _return.handle = _return.title.toLowerCase().replace(/[\s\,\-\.]/g,'-').replace(/[\,\.\?\-]+$/, '');
    _return.template = typeSettings.template+'.hbs';
    _return.path = '/' + typeSettings.rootPath + '/' + _return.handle;
    _return.permalink = process.env.NODE_ENV === 'development' ? settings.meta.dev_url+_return.path : settings.meta.production_url+_return.path;
  };
...
function partial(partial, action, cb){
  ...
  /**
   * Adds new post to start 
   * of array
   */
  if (!updating){
    _items.unshift(_partial)
  }
  ...
  //   var _a, _b;
  //   _a = a.split(/\-|[A-z]|\:|\./).splice(0,6);
  //   _b = b.split(/\-|[A-z]|\:|\./).splice(0,6);
  //   _a = Date.UTC(_a);
  //   _b = Date.UTC(_b);
  //   return _a - _b
  // });
